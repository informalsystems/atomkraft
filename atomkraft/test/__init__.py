...
def FileOption(help, default):
    return typer.Option(None, show_default = False, 
        help = f"{help} [grey30]\[default: set via [bold cyan]atomkraft {default}[/bold cyan]][/grey30]")

@app.command()

def trace(trace: typer.FileText = FileOption("trace to execute", "model"), 
...
@app.command()
def model(model: typer.FileText = FileOption("model used to generate traces", "model"),
          ...
          test: Optional[List[str]] = typer.Option(None, show_default = False, 
    ...
    tests = [t for ts in tests for t in ts]
    print(f'''
    Model: {model}
    Config: {config}
    Reactor: {reactor}
